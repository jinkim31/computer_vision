...
//    void        Trim(int nDummy){ Row() = (Row()-1 > nDummy ? nDummy : Row()); } //neglect elements from the nDummy time
    ...
    double      Average();
    double      Variance();
    double      Nearest(const double& dValue, int* npIdx);
...
public:
    ...
    KImageGray   Transform(const KMatrix& mHomography, KRect* rcpValid = 0)   const;
    KImageGray&  Transformed(const KMatrix& mHomography, KRect* rcpValid = 0);
...
public:
    ...
    void            MatchHistogram(const KImageColor& icRef,KImageColor& icIn);
    void            MatchHistogram(KImageGray& igIn);
    void            MatchHistogram(const KVector& vLookupR, const KVector& vLookupG, const KVector& vLookupB, KImageColor& icSrc);
...
//operations
public:
void                         HarrisCorner(const double& dThresh, const KImageGray& igIn, KStack<KCORNERPIXEL>& cornerStack);
