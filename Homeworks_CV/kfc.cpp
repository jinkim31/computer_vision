...
KImageGray KImageGray::Transform(const KMatrix& mHomography, KRect* rcpValid) const
{
    int         nRow = (rcpValid ? rcpValid->Height() : Row());
    int         nCol = (rcpValid ? rcpValid->Width()  : Col());
    int         nSx  = (rcpValid ? rcpValid->_nLeft   : 0);
    int         nSy  = (rcpValid ? rcpValid->_nTop    : 0);
    int         nEx  = (rcpValid ? rcpValid->_nRight  : nCol-1);
    int         nEy  = (rcpValid ? rcpValid->_nBottom : nRow-1);

    KImageGray  igOut(nRow, nCol);
    KVector     vUV;
    KMatrix     mHinv = ~mHomography;

    for(int r=nSy, rr=nRow, i=0; rr; r++, rr--, i++)
        for(int c=nSx, cc=nCol, j=0; cc; c++, cc--, j++)
        {
            vUV = mHinv * KVector((double)c,(double)r,1.0);
            if(vUV[2] == 0.0) continue;
            vUV /= vUV[2];

            if(vUV[0] < nSx || vUV[1] < nSy || vUV[0] > nEx || vUV[1] > nEy)
                continue;

            igOut._ppA[i][j] = BilinearInterpolation(vUV[0],vUV[1]);
        }

    return igOut;
}

KImageGray& KImageGray::Transformed(const KMatrix& mHomography, KRect* rcpValid)
{
    int         nRow = (rcpValid ? rcpValid->Height() : Row());
    int         nCol = (rcpValid ? rcpValid->Width()  : Col());
    int         nSx  = (rcpValid ? rcpValid->_nLeft   : 0);
    ...
    for(int r=nSy, rr=nRow, i=0; rr; r++, rr--, i++)
        for(int c=nSx, cc=nCol, j=0; cc; c++, cc--, j++)
        {
            ...
            if(vUV[0] < 0.0 || vUV[1] < 0.0 || vUV[0] > nEx || vUV[1] > nEy)
                continue;

            igOut._ppA[i][j] = BilinearInterpolation(vUV[0],vUV[1]);
        }

    this->Create(nRow,nCol,igOut.Address());
...
void KHisto::MatchHistogram(KImageGray& igSrc)
{
    ...
    for(int ic=igSrc.Row(),i=0; ic; ic--,i++)
        for(int jc=igSrc.Col(),j=0; jc; jc--,j++)
            igSrc[i][j] = (unsigned char)(npLookup[igSrc[i][j]]);

}

void KHisto::MatchHistogram(const KImageColor& icRef, KImageColor& icSrc)
{
    //color split
    KImageGray igpRef[3], igpSrc[3];
    ...
    for(int n=0; n<3; n++)
    {
        ...
        for(int ic=igpSrc[n].Row(),i=0; ic; ic--,i++)
            for(int jc=igpSrc[n].Col(),j=0; jc; jc--,j++)
                igpSrc[n][i][j] = (unsigned char)(npLookup[igpSrc[n][i][j]]);
    }


    //output
    icSrc.FromRGB(igpSrc[_RED], igpSrc[_GREEN], igpSrc[_BLUE]);
}

void KHisto::MatchHistogram(const KVector& vLookupR, const KVector& vLookupG, const KVector& vLookupB, KImageColor& icSrc)
{
    //transform
    for(int ic=icSrc.Row(),i=0; ic; ic--,i++)
        for(int jc=icSrc.Col(),j=0; jc; jc--,j++)
        {
            icSrc[i][j].r = (unsigned char)(vLookupR[icSrc[i][j].r]);
            icSrc[i][j].g = (unsigned char)(vLookupG[icSrc[i][j].g]);
            icSrc[i][j].b = (unsigned char)(vLookupB[icSrc[i][j].b]);
        }
}
...
void KCorner::HarrisCorner(const double& dThresh, const KImageGray& igIn, KStack<KCORNERPIXEL>& cornerStack)
{
    //gradients
